...
in
{
  defaults =
    ...
    {
      options.deployment.virtualbox = lib.mkOption {
      ...
      config = lib.mkIf (config.deployment.targetEnv == "virtualbox") {
        deployment.targetHost =
          if resources != null then
            (lib.head resources.virtualbox_vm.${name}.network_adapter).ipv4_address
          else
            tf.ref "element(virtualbox_vm.${name}.network_adapter, 0).ipv4_address";
        ...
        deployment.virtualbox = {
          ...
          provisioner = lib.mkIf config.deployment.provisionSSHKey {
            local-exec = {
              command = ''
  ...
  resource =
    { nodes, lib, ... }:
    let
      nodes' = lib.filterAttrs (_: node: node.config.deployment.targetEnv == "virtualbox") nodes;
    in
    {
      virtualbox_vm = lib.mapAttrs (_: node: node.config.deployment.virtualbox) nodes';
    };
}
// lib.mapAttrs (
  _: node:
