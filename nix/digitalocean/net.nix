...
let
  /*
    ...
    Examples:
      ...
      => "192.168.70.9"
  */
  prettyIp = addr: lib.concatStringsSep "." (builtins.map builtins.toString addr);
  ...
  bitMaskToSubnetMask =
    ...
    lib.genList (
      ...
      # First non-full part generation
        ...
        _genPartialMask (lib.mod bitMask octetBits)
    ) numOctets;
  ...
  */
  subnetMaskToBitMask =
    subnetMask:
    let
      partialBits = octet: if octet == 0 then 0 else (lib.mod octet 2) + partialBits (octet / 2);
    in
    builtins.foldl' (x: y: x + y) 0 (builtins.map partialBits subnetMask);
  ...
  */
  cidrToIpAddress =
    ...
    let
      ...
      parsed = builtins.map lib.toInt (builtins.match "([0-9]+)\\.([0-9]+)\\.([0-9]+)\\.([0-9]+)" addr);
      checkBounds = octet: (octet >= 0) && (octet <= 255);
    in
    if (builtins.all checkBounds parsed) then
      parsed
    else
      builtins.throw "IP ${prettyIp addr} has out of bounds octet(s)";
  ...
  */
  cidrToBitMask =
    ...
    let
      ...
      checkBounds = mask: (mask >= 0) && (mask <= 32);
    in
    if (checkBounds mask) then mask else builtins.throw "Bitmask ${builtins.toString mask} is invalid.";
  ...
  /*
    ...
    Examples:
      ...
      => [ 255 254 0 0 ]
  */
  cidrToSubnetMask = cidr: bitMaskToSubnetMask (cidrToBitMask cidr);
  ...
  */
  getBroadcastAddress =
    networkId: subnetMask: lib.zipListsWith (nid: mask: 255 - mask + nid) networkId subnetMask;
  ...
  */
  ...
  /*
    ...
    Examples:
      ...
      => "192.168.70.9/15"
  */
  ipAndSubnetMaskToCidr = addr: subnetMask: ipAndBitMaskToCidr addr (subnetMaskToBitMask subnetMask);
