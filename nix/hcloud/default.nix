...
in
{
      ...
      config = lib.mkIf (config.deployment.targetEnv == "hcloud") {
        deployment.targetHost =
          if resources != null then
            resources.hcloud_server.${name}.ipv4_address
          else
            tf.ref "hcloud_server.${name}.ipv4_address";
  ...
  resource =
    ...
    lib.optionalAttrs (nodes' != { }) {
      hcloud_server = lib.mapAttrs (
        _: node:
        node.config.deployment.hcloud
        // {
      ...
      ssh_resource.teraflops = {
        ...
        private_key = lib.mkIf (config.resource.tls_private_key.teraflops or null != null) (
          tf.ref "each.value.provisionSSHKey ? tls_private_key.teraflops.private_key_openssh : null"
        );
    ...
    // lib.optionalAttrs (filesystems' != [ ]) {
      hcloud_volume = lib.genAttrs' filesystems' (
        data: lib.nameValuePair data.hcloud.name (builtins.removeAttrs data.hcloud [ "automount" ])
      );
      hcloud_volume_attachment = lib.genAttrs' filesystems' (
        data:
        lib.nameValuePair "${data.hcloud.name}-on-${data.server}" {
    ...
    // lib.optionalAttrs (addresses' != [ ]) {
      ...
      hcloud_floating_ip_assignment = lib.genAttrs' addresses' (
        v:
        lib.nameValuePair "${v.hcloud.name}-on-${v.server}" {
