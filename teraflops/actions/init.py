...
def register_action(subparsers):
    ...
    parser.add_argument(
        '--migrate-state', action='store_true', help='reconfigure a backend, and attempt to migrate any existing state'
    )
    parser.add_argument(
        '--reconfigure', action='store_true', help='reconfigure a backend, ignoring any saved configuration'
    )
    parser.add_argument(
        '--upgrade',
        action='store_true',
        help='install the latest module and provider versions allowed within configured constraints, overriding the '
