                    ...
                    if name in selected:
                        tg.create_task(
                            pipeline(
                                ...
                                deployment,
                                terraform_json,
                                drvs[name] if args.with_drvs else None,
                                private_key,
                            )
                        )
...
def register_action(subparsers):
    parser = subparsers.add_parser(
        'activate',
        parents=[parsers.on(), parsers.no_keys(), parsers.with_drvs()],
        help='apply configurations on remote nodes',
    )
    parser.set_defaults(func=run)

    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        '--boot', action='store_const', dest='flag', const='boot', help='activate nodes on the next reboot'
    )
    group.add_argument(
        '--reboot',
        action='store_const',
        dest='flag',
        const='reboot',
        help='reboots nodes, activate, and wait for the nodes to come back up',
    )
    group.add_argument(
        '--test',
        action='store_const',
        dest='flag',
        const='test',
        help='build and activate the new configuration without making it the default on boot',
    )
    group.add_argument(
        '--switch',
        action='store_const',
        dest='flag',
        const='switch',
        help='build and activate the new configuration making it the default on boot',
    )
    group.add_argument(
        '--dry-run',
        action='store_const',
        dest='flag',
        const='dry_run',
        help='show what changes would be performed by the activation',
    )
