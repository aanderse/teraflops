...
def register_action(subparsers):
    parser = subparsers.add_parser(
        'reboot', parents=[parsers.on(), parsers.no_keys()], help='reboot all nodes in the deployment'
    )
    parser.set_defaults(func=run)
    parser.add_argument(
        '--no-wait', action='store_true', help='do not wait until the nodes are up again - implies --no-keys argument'
    )
