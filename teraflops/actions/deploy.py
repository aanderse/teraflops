...
async def wait_for_node(ctx, name, node, private_key):
    ...
    while True:
        ...
        proc = await asyncio.create_subprocess_exec(
            *ssh.cmd(node, ['cat', '/proc/sys/kernel/random/boot_id'], private_key, ['-o', 'ConnectTimeout=10']),
...
def register_action(subparsers):
    parser = subparsers.add_parser(
        'deploy', parents=[parsers.confirm(), parsers.on(), parsers.no_keys()], help='deploy the configuration'
    )
    parser.set_defaults(func=run)
    parser.add_argument(
        '--reboot', action='store_true', help='reboots nodes after activation and waits for them to come back up'
    )
